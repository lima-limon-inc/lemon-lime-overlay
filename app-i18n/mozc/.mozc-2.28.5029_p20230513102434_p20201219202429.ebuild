...
if [[ "${PV}" == "9999" ]]; then
	...
	# This is the first commit after release 2.28.5029.102
	MOZC_GIT_REVISION="961949a1561348031fe03f72de46d480695734d2"

	# MOZC_DATE will be equal to whatever comes after the first "_p" in the
	# package name
	MOZC_DATE="${PV#*_p}"

	# MOZC_DATE will be equal to whatever comes before the second "_p" in the
	# package name
	MOZC_DATE="${MOZC_DATE%%_p*}"

	# This commits has the latest available tag
	FCITX_MOZC_GIT_REVISION="1ea089debc31ff216473369ad71c08318384ee06"
	FCITX_MOZC_DATE="${PV#*_p}"
	FCITX_MOZC_DATE="${FCITX_MOZC_DATE#*_p}"
	FCITX_MOZC_DATE="${FCITX_MOZC_DATE%%_p*}"

	# Latest commit in hiroyuki-komatsu's japanese_usage_dictionary repository
	JAPANESE_USAGE_DICTIONARY_GIT_REVISION="a4a66772e33746b91e99caceecced9a28507e925"

	# This refers to the date the commit in JAPANESE_USAGE_DICTIONARY_GIT_REVISION
	# was commited
...
src_configure() {
	...
	execute "${PYTHON}" build_mozc.py --use_gyp_for_ibus_build gyp \
